...
// Editing a book's full text re-splits every paragraph, so the numeric anchors
// on existing comments (paragraph_idx + char offsets) stop meaning anything.
// But each comment still carries the *text* it was drawn on, so we re-find that
// text in the new paragraphs and move the comment there. A comment survives when
// its selected_text is located in some new paragraph (nearest to the old index
// wins on ties); replies without their own selection follow their thread's
// nearest surviving ancestor. Only text that no longer exists anywhere is lost.
//
// Returns { updates: Map<oldId, {paragraph_idx, sel_start_idx, sel_end_idx}>,
//           reparent: Map<id, newReplyTo|null>, orphanIds: number[] }.
export function reanchorComments(comments, newParagraphs) {
  const byId = new Map(comments.map(c => [c.id, c]));
  const survived = new Map();

  const findByText = (selText, oldParaIdx) => {
    if (!selText) return null;
    let best = null, bestDist = Infinity;
    for (const p of newParagraphs) {
      const pos = p.content.indexOf(selText);
      if (pos < 0) continue;
      const dist = Math.abs(p.idx - oldParaIdx);
      if (dist < bestDist) { bestDist = dist; best = { paragraph_idx: p.idx, sel_start_idx: pos, sel_end_idx: pos + selText.length }; }
    }
    return best;
  };

  // Pass 1: anchor every comment that carries a single-paragraph text selection.
  // Cross-paragraph selections (sel_end_para_idx set) can't be matched as one
  // substring, so they fall through to the reply-follow pass or become orphans.
  for (const c of comments) {
    if (c.selected_text && c.sel_end_para_idx == null) {
      const a = findByText(c.selected_text, c.paragraph_idx);
      if (a) survived.set(c.id, a);
    }
  }

  const rootOf = (c) => {
    let cur = c; const seen = new Set();
    while (cur.reply_to != null && byId.has(cur.reply_to) && !seen.has(cur.id)) { seen.add(cur.id); cur = byId.get(cur.reply_to); }
    return cur;
  };

  // Pass 2: a reply (or whole-paragraph note in a thread) with no locatable text
  // of its own rides along with its thread root, if that root survived.
  for (const c of comments) {
    if (survived.has(c.id) || c.reply_to == null) continue;
    const anchor = survived.get(rootOf(c).id);
    if (anchor) survived.set(c.id, { paragraph_idx: anchor.paragraph_idx, sel_start_idx: null, sel_end_idx: null });
  }

  // A survivor whose direct reply_to was orphaned is re-pointed to its nearest
  // surviving ancestor (or detached to top-level) so no reply dangles.
  const reparent = new Map();
  for (const c of comments) {
    if (!survived.has(c.id) || c.reply_to == null || survived.has(c.reply_to)) continue;
    let anc = byId.get(c.reply_to); const seen = new Set();
    while (anc && !survived.has(anc.id) && anc.reply_to != null && !seen.has(anc.id)) { seen.add(anc.id); anc = byId.get(anc.reply_to); }
    reparent.set(c.id, anc && survived.has(anc.id) ? anc.id : null);
  }

  const orphanIds = comments.filter(c => !survived.has(c.id)).map(c => c.id);
  return { updates: survived, reparent, orphanIds };
}
...
export const CONTEXT_CHARS_MIN = 0;
...
export async function handleRequest(req, res, opts = {}) {
  ...
  // PUT /v1/settings — persist context_chars (0..5000). Same value feeds both
  ...
  // GET /v1/books/:id/raw — full plain text (paragraphs joined by blank lines),
  // for the editor to prefill. Round-trips through smartSplit: one paragraph per
  // block, so re-saving unchanged text reproduces the same split.
  if (req.method === 'GET' && req.url.match(/^\/v1\/books\/\d+\/raw(?:\?|$)/)) {
    if (roomGate(req, res)) return true;
    try {
      const id = parseInt(req.url.split('/')[3]);
      const db = getDb(true);
      ...
      if (!book) { db.close(); json(res, 404, { error: 'book not found' }); return true; }
      const paras = db.prepare('SELECT content FROM book_paragraphs WHERE book_id = ? ORDER BY idx').all(id);
      db.close();
      json(res, 200, { id: book.id, title: book.title, content: paras.map(p => p.content).join('\n\n') });
    } catch (e) { json(res, 500, { error: e.message }); }
    return true;
  }

  // PATCH /v1/books/:id — rename and/or replace the whole text of an existing
  // book. `title` alone is a pure rename (annotations/progress untouched).
  // Sending `content` re-splits the text: every paragraph index shifts, so the
  // old comments/progress/bookmark anchors would point at the wrong lines —
  // they are cleared. The web client warns before sending content when a book
  // has annotations. Cover/images are left alone.
  if (req.method === 'PATCH' && req.url.match(/^\/v1\/books\/\d+$/)) {
    ...
    try {
      ...
      if (!hasTitle && !hasContent) { json(res, 400, { error: 'title or content required' }); return true; }
      const newTitle = hasTitle ? String(body.title).trim() : null;
      if (hasTitle && !newTitle) { json(res, 400, { error: 'title cannot be empty' }); return true; }

      let paragraphs = null;
      if (hasContent) {
        paragraphs = smartSplit(String(body.content));
        if (paragraphs.length === 0) { json(res, 400, { error: 'no paragraphs extracted' }); return true; }
      }
      ...
      db.transaction(() => {
        if (hasTitle) db.prepare('UPDATE books SET title = ? WHERE id = ?').run(newTitle, id);
        if (hasContent) {
          const comments = db.prepare('SELECT id, paragraph_idx, sel_start_idx, sel_end_idx, sel_end_para_idx, selected_text, reply_to FROM book_comments WHERE book_id = ?').all(id);

          db.prepare('DELETE FROM book_paragraphs WHERE book_id = ?').run(id);
          const ins = db.prepare('INSERT INTO book_paragraphs (book_id, idx, content) VALUES (?, ?, ?)');
          const newParagraphs = paragraphs.map((content, idx) => ({ idx, content }));
          for (const p of newParagraphs) ins.run(id, p.idx, p.content);
          db.prepare('UPDATE books SET total_paragraphs = ? WHERE id = ?').run(paragraphs.length, id);

          // Move surviving comments onto their new text position; drop the rest.
          const { updates, reparent, orphanIds } = reanchorComments(comments, newParagraphs);
          const upd = db.prepare('UPDATE book_comments SET paragraph_idx = ?, sel_start_idx = ?, sel_end_idx = ?, sel_end_para_idx = NULL WHERE id = ?');
          for (const [cid, a] of updates) upd.run(a.paragraph_idx, a.sel_start_idx, a.sel_end_idx, cid);
          const rep = db.prepare('UPDATE book_comments SET reply_to = ? WHERE id = ?');
          for (const [cid, newReplyTo] of reparent) rep.run(newReplyTo, cid);
          const del = db.prepare('DELETE FROM book_comments WHERE id = ?');
          for (const oid of orphanIds) del.run(oid);
          retained = updates.size;
          dropped = orphanIds.length;

          // Progress/bookmark anchor to a paragraph index, not text; the offset
          // may now overrun its (rewritten) paragraph. Keep the row but clamp the
          // index into range and reset the offset so it can never point past the
          // paragraph's end.
          const lastIdx = Math.max(0, paragraphs.length - 1);
          db.prepare('UPDATE book_progress SET page = MIN(page, ?), char_offset = 0 WHERE book_id = ?').run(lastIdx, id);
          db.prepare('UPDATE book_bookmarks SET page = MIN(page, ?), paragraph_idx = MIN(paragraph_idx, ?), char_offset = 0 WHERE book_id = ?').run(lastIdx, lastIdx, id);
        }
      })();
      const updated = db.prepare('SELECT id, title, total_paragraphs FROM books WHERE id = ?').get(id);
      db.close();
      json(res, 200, { ok: true, book: updated, content_replaced: hasContent, retained_annotations: retained, dropped_annotations: dropped });
    } catch (e) { json(res, 500, { error: e.message }); }
    return true;
  }
  ...
  if (req.method === 'DELETE' && req.url.match(/^\/v1\/books\/\d+$/)) {
    try {
      ...
      db.close();
      fs.rmSync(getImageDir(id), { recursive: true, force: true });
